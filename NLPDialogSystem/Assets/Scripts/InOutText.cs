...
public class InOutText : MonoBehaviour {
    ...
    void Start () {
        ...
        //one = new Character(Application.dataPath+"/Resources/base/"+ BaseKnowledge, NameofGamePerson, lang);
        one = new Character();
        ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	void FixedUpdate () {
        if (Input.GetKeyDown(KeyCode.E) && Marcket.EnterMarcket)
        {
            //ShowGO();
            if (Marcket.EnterMarcket)
            {
                one = new Character(Application.dataPath + "/Resources/base/" + BaseKnowledge, NameofGamePerson, lang);
                one.setEvent("chapter1");
                ...
                ShowAnsvers();
                Marcket.EnterMarcket = false;
            }
            else
            {
                ClearAnsvers();
                output.text = "";
                Marcket.ExitMarcket = false;
            }
            enter = true;
        }
        else
        {
            if (Marcket.ExitMarcket)
            {
                ClearAnsvers();
                output.text = "";
                Marcket.ExitMarcket = false;
    ...
    public void SubmitInput(string arg0)
    {
        if (!arg0.Equals(""))
        {
            ...
            string request = one.CompliteAnsver(arg0);
            if (request.Contains("action"))
            {
                if (!request.Contains("error"))
                {
                    request = request.Remove(request.IndexOf("action"), request.IndexOf("\n"));
                    output.text = newText + "\n" + one.Name + ": " + "Сделано." + request + "\n";
                }
                else
                {
                    request = request.Remove(request.IndexOf("action"), request.IndexOf("\n"));
                    output.text = newText + "\n" + one.Name + ": " + "Не могу выполнить." + request + "\n";
                }
            }
            else
            {
   ...
   /* public void HideGO()
    {
        ...
        ShowHideGO[0].gameObject.GetComponent<Canvas>().sortingOrder = -5;
        for (int i = 0; i < ShowHideGO[1].gameObject.transform.childCount; i++)
            ShowHideGO[1].transform.GetChild(i).GetComponentInChildren<SpriteRenderer>().sortingOrder = -5;
    }
    //public void ShowGO()
    {
        ShowHideGO[0].gameObject.GetComponent<Canvas>().sortingOrder = 5;
